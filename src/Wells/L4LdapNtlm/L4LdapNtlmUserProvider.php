...
class L4LdapNtlmUserProvider implements UserProviderInterface
{
    ...
    public function clean(array $entry)
    {
        ...
        $entry['group'] = '';
        $entry['roles'] = array();
        $entry['memberof'] = array();

        $this->getMembersRecursively($entry, $entry['dn']);
     ...
     * @param $group
     */
    protected function getMembersRecursively(array &$entry, $group)
    {
        $subgroups = $this->getMembers($group);
        if ($subgroups != null) {
            foreach ($subgroups as $subgroup) {
                $subgroup_name = trim(str_replace('CN=', '', explode(",", $subgroup)[0]));
                array_push($entry['memberof'], $subgroup_name);
                array_push($entry['roles'], strtoupper($subgroup_name));
                $this->getMembersRecursively($entry, $subgroup);
            }
        }
    }
